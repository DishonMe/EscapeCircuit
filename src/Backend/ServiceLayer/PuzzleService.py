...
class PuzzleService:
    ...
    """
    def __init__(self, puzzle_repo: PuzzleRepo, user_repo: UserRepo, auth_service: AuthService, solve_repo: SolveRepo | None = None, arsenal_service=None, xp_service=None):
    ...
    def publish(self, session_token: str, puzzle_id: int) -> dict:
        ...
        with transaction(self.repo.conn):
            if not is_admin:
                # Per-user admin-configurable limit (falls back to level-based default)
                pub_limit, _ = self._get_effective_limits(user)
                current_count = self.repo.count_by_creator_and_status(p.creator_user_id, PuzzleStatus.PUBLISHED)
                if p.status != PuzzleStatus.PUBLISHED and current_count >= pub_limit:
                    raise ValidationError(
