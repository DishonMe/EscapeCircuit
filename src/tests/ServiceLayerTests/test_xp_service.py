...
class TestXPServicePuzzleLimits:
    ...
    # --- published limit ---
    # With beta's level formula: level = floor(sqrt(xp / 100)) + 1
    # Level 10 min XP = 8100, level 11 = 10000, ..., level 15 = 19600

    def test_published_limit_default_low_level(self):
        # Levels 1-9 (XP < 8100) → base of 5
        assert self.service.get_puzzle_published_limit(0) == 5
        assert self.service.get_puzzle_published_limit(8099) == 5

    def test_published_limit_level_10(self):
        # 8100 XP → level 10 → 5 + 2 = 7
        assert self.service.get_puzzle_published_limit(8100) == 7

    def test_published_limit_level_11(self):
        # 10000 XP → level 11 → 5 + 4 = 9
    ...
    def test_published_limit_override_zero(self):
        assert self.service.get_puzzle_published_limit(8100, override=0) == 0

    def test_published_limit_override_none_uses_level(self):
        assert self.service.get_puzzle_published_limit(8100, override=None) == 7

    # --- unpublished limit ---

    def test_unpublished_limit_matches_published_by_default(self):
        for xp in [0, 500, 8100, 10000, 19600]:
    ...
    def test_unpublished_limit_independent_override(self):
        ...
        assert self.service.get_puzzle_unpublished_limit(8100, override=10) == 10
        assert self.service.get_puzzle_published_limit(8100, override=None) == 7
