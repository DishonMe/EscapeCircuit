...
export const PuzzleDetailsDialog = ({
  ...
  const [activeTab, setActiveTab] = useState<'instructions' | 'custom'>(
    'instructions',
  );
        ...
        {puzzle ? (
          <div className="max-h-[60vh] overflow-y-auto">
                ...
                <button
                  ...
                  className={`px-3 py-2 text-sm font-medium transition-colors ${
                    activeTab === 'instructions'
                      ? 'mb-[-2px] border-b-2 border-foreground text-foreground'
            ...
            {activeTab === 'custom' &&
              ...
              puzzle.customComponents.length > 0 && (
                        ...
                        <div className="mb-3 flex items-start justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              Cost: {piece.cost} • Inputs: {piece.num_inputs} •
                              Outputs: {piece.num_outputs}
                            </p>
                          </div>
                        </div>
                          ...
                          <div className="overflow-x-auto">
                            <table className="w-full border-collapse text-xs text-foreground">
                              <thead>
                                <tr>
                                  {Object.keys(
                                    truthTable[Object.keys(truthTable)[0]] ||
                                      {},
                                  ).map((key) => (
                                    <th
                                      key={key}
                                      className="border border-border bg-secondary px-2 py-1 text-left font-medium"
                                    >
                                      {key}
                                    </th>
                                  ))}
                              ...
                              <tbody>
                                {Object.entries(truthTable).map(
                                  ([input, outputs]: [string, any]) => (
                                    <tr key={input}>
