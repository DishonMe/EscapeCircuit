...
const buildDefaultTruthTable = (
  ...
  for (let i = 0; i < numCombinations; i++) {
    const inputKey = i.toString(2).padStart(numInputs, '0');
...
export default function CreatePuzzleForm() {
  ...
  const [initialBoardPlaced, setInitialBoardPlaced] = useState<
    PlacedGridComponent[]
  >([]);
  const [initialBoardWires, setInitialBoardWires] = useState<Wire[]>([]);
  const [initialBoardSelectedComponent, setInitialBoardSelectedComponent] =
    useState<SelectedComponentState>({ mode: 'none' });
  const [
    initialBoardDraggedPaletteComponentId,
    setInitialBoardDraggedPaletteComponentId,
  ...
  const [viewingTruthTableFor, setViewingTruthTableFor] = useState<
    string | null
  >(null);
  const [viewingCustomPieceTruthTable, setViewingCustomPieceTruthTable] =
    useState<number | null>(null);

  // State for solution structure guide modal
  const [guideModal, setGuideModal] = useState<
    'basic' | 'solution' | 'python-tests' | null
  >(null);
  ...
  const selectedArsenalPieces = useMemo(() => {
    ...
    const filtered = myArsenalData.filter((piece) => {
      ...
      const isIncluded =
        data.basic.allowedArsenalComponentIds.includes(pieceIdStr);
      return isIncluded;
    });
    ...
    const result = filtered.map((piece) => {
      ...
      try {
        ...
        if (structure.inputs) {
          num_inputs = Array.isArray(structure.inputs)
            ? structure.inputs.length
            : 1;
        }
        if (structure.outputs) {
          num_outputs = Array.isArray(structure.outputs)
            ? structure.outputs.length
            : 1;
        }
      } catch (e) {
  ...
  const customLimitTargets = useMemo(
    () =>
      gateLimitTargets.filter(
        (target) =>
          target.category === 'custom' || target.category === 'custom-total',
      ),
    [gateLimitTargets],
  );
  ...
  useEffect(() => {
    if (
      customPieceForm.numInputs !== prevNumInputs ||
      customPieceForm.numOutputs !== prevNumOutputs
    ) {
  ...
  const handleArsenalDisplayModeChange = (
    componentId: string,
    mode: 'circuit' | 'description',
  ) => {
    setData((prev) => ({
      ...prev,
      basic: {
  ...
  const exportSolution = useCallback(async () => {
    ...
    setData((prev) => ({
      ...
      basic:
        prev.basic.creator_budget === null
  ...
  const handleSubmit = async () => {
    ...
    if (data.basic.description.length > MAX_PUZZLE_DESCRIPTION_LENGTH) {
      alert(
        `Description must be at most ${MAX_PUZZLE_DESCRIPTION_LENGTH} characters`,
      );
      return;
    }
    if (
      ...
      alert(
        'Add at least one available component source: gate type, custom piece, or shared arsenal piece',
      );
      return;
    }
    ...
    if (utf8ByteLength(data.instructions) > MAX_PUZZLE_INSTRUCTIONS_BYTES) {
      alert(
        `Instructions must be at most ${MAX_PUZZLE_INSTRUCTIONS_BYTES} bytes`,
      );
      return;
    }
    ...
    const sharedArsenalNames = new Set(
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars -- discard id when serializing
      ...
      const configData: {
        ...
        puzzle: {
          ...
          gate_quotas:
            Object.keys(data.basic.gateQuotas).length > 0
              ? data.basic.gateQuotas
              : undefined,
          ...
          initial_board:
            initialBoardPlaced.length > 0 || initialBoardWires.length > 0
              ? {
        ...
        <div className="mb-8">
          {activeTab === 'basic' && (
            <div className="space-y-6">
              ...
              <div>
                ...
                />
                <div className="mt-1 text-right text-[11px] text-muted-foreground">
                  {data.basic.description.length}/
                  {MAX_PUZZLE_DESCRIPTION_LENGTH}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label
                    htmlFor="cp-field-budget"
                    className="mb-2 flex items-center gap-1 text-[13px] font-medium text-foreground"
                  >
                    Budget *
                    <InfoPopup>
                      <p className="mb-1 font-medium text-foreground">Budget</p>
                      <p>
                        The max total gate cost solvers can use. Solvers who
                        exceed this cannot submit.
                      </p>
                      <p className="mt-1">
                        <span className="font-medium text-foreground">
                          Creator Budget
                        </span>{' '}
                        is your solution&apos;s gate cost and is auto-filled
                        when you export your solution. Solvers who match or beat
                        it earn a better medal.
                      </p>
                    </InfoPopup>
                  </label>
                  <input
                ...
                <div>
                  ...
                  >
                    ...
                    <InfoPopup>
                      ...
                      <p>
                        The circuit grid dimensions for solvers. Default is 15
                        rows x 30 columns.
                      </p>
                      <p className="mt-1">
                        Larger boards give solvers more room. Smaller boards
                        increase difficulty.
                      </p>
                    </InfoPopup>
                  </label>
                  <input
                    id="cp-field-board-rows"
                    type="number"
                    min="5"
                    max="60"
                      ...
                      handleBasicChange(
                        ...
                        Math.max(
                          ...
                          Math.min(
                            60,
                            parseInt(
                              e.target.value || `${DEFAULT_BOARD_ROWS}`,
                              10,
                            ),
                          ),
                        ),
                      ...
                      handleBasicChange(
                        ...
                        Math.max(
                          ...
                          Math.min(
                            80,
                            parseInt(
                              e.target.value || `${DEFAULT_BOARD_COLS}`,
                              10,
                            ),
                          ),
                        ),
              ...
              <div>
                <p className="mb-2 block text-[13px] font-medium text-foreground">
                  Gate Set *
                </p>
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-muted-foreground">
                  If unchecked, solver personal arsenal is disabled.
                  Creator-shared arsenal pieces selected below are still
                  available.
                </p>
              </div>
              ...
              {myArsenalData && myArsenalData.length > 0 && (
                <div>
                  ...
                  <p className="mb-3 text-[11px] text-muted-foreground">
                    Select your arsenal components that are explicitly shared
                    with solvers for this puzzle. These shared components remain
                    available even if solver personal arsenal is disabled.
                  </p>
                  <div className="max-h-48 space-y-2 overflow-y-auto rounded-lg border border-border bg-secondary/30 p-3">
                    {myArsenalData.map((piece) => {
                      const isIncluded =
                        data.basic.allowedArsenalComponentIds.includes(
                          String(piece.id),
                        );
                      const displayMode =
                        data.basic.arsenalComponentDisplayModes[
                      ...
                      return (
                        <div
                          key={piece.id}
                          className="flex items-center gap-2 rounded p-2 text-[13px] hover:bg-secondary/50"
                        >
                          <input
                            type="checkbox"
                            checked={isIncluded}
                            onChange={(e) =>
                              handleArsenalSelection(
                                String(piece.id),
                                e.target.checked,
                              )
                            }
                          ...
                          {isIncluded && (
                            <div className="flex gap-1">
                              <button
                                type="button"
                                onClick={() =>
                                  handleArsenalDisplayModeChange(
                                    String(piece.id),
                                    'circuit',
                                  )
                                }
              ...
              {gateLimitTargets.length > 0 && (
                <div className="space-y-4">
                  ...
                  <p className="mb-2 text-[11px] text-muted-foreground">
                    Ordered by category. Basic and custom limits support
                    min/max. Non-shared arsenal limits are max-only.
                  </p>
                          ...
                          >
                            <div className="mb-2 flex items-center justify-between gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  setViewingTruthTableFor(target.name)
                                }
                                className="p-1 text-foreground/40 transition-opacity hover:text-foreground/70"
                            ...
                            <div className="space-y-1">
                              ...
                              <input
                                ...
                                min="0"
                                value={
                                  data.basic.minGateQuotas[target.name] ?? ''
                                }
                                onChange={(e) => {
                                  const newMinQuotas = {
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="mb-2 flex items-center gap-2 text-[13px] font-medium text-foreground">
                    Outputs *
                    <button
                      type="button"
                      onClick={() => setGuideModal('basic')}
                      className="shrink-0 p-0.5 text-foreground/40 transition-opacity hover:text-foreground/70"
                        ...
                        <div className="mb-4 space-y-2">
                          {data.basic.outputs.map((outputName) => (
                            ...
                            >
                              ...
                              <select
                                value={
                                  testCaseForm.expectedOutputs?.[outputName] ??
                                  0
                                }
                                onChange={(e) =>
                                  setTestCaseForm((prev) => ({
          ...
          {activeTab === 'instructions' && (
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                <label htmlFor="cp-field-12" className="block font-semibold">
                  Instructions (LaTeX) *
                </label>
                <textarea
                  id="cp-field-12"
                ...
                <div className="text-[13px] text-muted-foreground">
                  ...
                  {'\\begin{itemize}'} for lists, {'\\begin{tabular}'} for
                  tables
                </div>
                <div className="text-right text-[11px] text-muted-foreground">
                  {utf8ByteLength(data.instructions)}/
                  {MAX_PUZZLE_INSTRUCTIONS_BYTES} bytes
                </div>
              </div>
              <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-border bg-secondary/50 p-4">
                <h3 className="mb-2 inline-flex items-center gap-2 font-semibold text-foreground">
                  ...
                  Design Your Initial Board (Pre-placed Locked Components)
                </h3>
                <div className="mb-3 space-y-2 text-[13px] text-foreground">
                  <p>
                    <strong>Instructions:</strong> Place the components that
                    will be pre-placed for the solver. All components you place
                    here are automatically locked and will:
                  </p>
                  <ul className="ml-2 list-inside list-disc space-y-1">
                    <li>Be immovable (cannot be dragged by solver)</li>
                    <li>Be undeletable (cannot be removed by solver)</li>
                    <li>
                      Be mandatory (must be connected to validate the solution)
                    </li>
                    <li>Be counted in the puzzle cost</li>
                  </ul>
                  <p className="mt-2 font-semibold text-blue-700">
                    Drag and place components. Locked components are shown with
                    a lock badge and dashed wires.
                  </p>
                </div>
              </div>
                ...
                <div className="overflow-y-auto border-r bg-secondary/50 p-3">
                  ...
                  selectedArsenalPieces.length === 0 ? (
                    <p className="text-[11px] text-muted-foreground">
                      Select gates in &quot;Basic Info&quot; tab, create pieces
                      in &quot;Custom Pieces&quot; tab, or select Arsenal pieces
                      in &quot;Basic Info&quot; tab
                    </p>
                  ) : (
                    <div className="space-y-2">
                      ...
                      {data.basic.gateSet.map((gateName) => (
                        <div
                          ...
                          onDragEnd={() =>
                            setInitialBoardDraggedPaletteComponentId(null)
                          }
                          className="flex cursor-move items-center gap-2 rounded-lg border border-border bg-card p-2 transition hover:bg-secondary/50"
                        >
                        ...
                        <div className="my-2 border-t border-border/50 pt-2">
                          ...
                          {customPieces.map((piece) => (
                            <div
                              ...
                              className="flex cursor-move items-center gap-2 rounded-lg border border-border/60 bg-amber-50/30 p-2 transition hover:bg-amber-50/60"
                            >
                              <span className="text-[13px] font-medium text-foreground">
                                {piece.name}
                              </span>
                              <div className="flex-1" />
                              <div className="shrink-0 text-[10px] text-muted-foreground">
                                cost {piece.cost} · pins{' '}
                                {piece.num_inputs + piece.num_outputs}
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="my-2 border-t border-border/50 pt-2">
                          ...
                          {selectedArsenalPieces.map((piece) => (
                            <div
                              ...
                              className="flex cursor-move items-center gap-2 rounded-lg border border-border/60 bg-blue-50/30 p-2 transition hover:bg-blue-50/60"
                            >
                              <span className="text-[13px] font-medium text-foreground">
                                {piece.name}
                              </span>
                    ...
                    {data.testCases.length > 0 ? (
                      data.testCases.map((tc, i) => {
                        if (tc.kind === 'blackbox') {
                          return (
                            <li key={i}>
                              Test {i}: Input {JSON.stringify(tc.inputs)} →
                              Output {JSON.stringify(tc.expectedOutputs)}
                            </li>
                          );
                        }
                        return null;
                    ...
                    <div className="space-y-2">
                      ...
                      {data.basic.gateSet.map((gateName) => (
                        ...
                        >
                          ...
                          <div className="flex-1" />
                          <div className="shrink-0 text-[10px] text-muted-foreground">
                            cost {GATE_PROPERTIES[gateName]?.cost ?? 1} · pins{' '}
                            {GATE_PROPERTIES[gateName]?.pins ?? 3}
                          </div>
                        </div>
                      ))}
...
def run_tests(solution):
    ...
    test_circuit_structure()`}
          ...
          {activeTab === 'custom-pieces' && (
            <div className="space-y-6">
              <div className="rounded-xl border border-border bg-secondary/50 p-4">
                <h3 className="mb-2 font-semibold">Create Custom Piece</h3>
                <p className="mb-4 text-[13px] text-muted-foreground">
                  Create special circuit pieces that solvers can use in this
                  puzzle. Limited to 5 inputs and 3 outputs.
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="h-20 w-full resize-none rounded-lg border border-border bg-transparent p-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-ring"
                    />
                  </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCustomPieceForm({
                            ...customPieceForm,
                            numInputs: parseInt(e.target.value) || 1,
                          })
                        }
                        className="w-full rounded-lg border border-border bg-transparent p-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-ring"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCustomPieceForm({
                            ...customPieceForm,
                            numOutputs: parseInt(e.target.value) || 1,
                          })
                        }
                        className="w-full rounded-lg border border-border bg-transparent p-3 text-[13px] focus:outline-none focus:ring-1 focus:ring-ring"
                      />
                  ...
                  {customPieceForm.numInputs > 0 &&
                    customPieceForm.numOutputs > 0 && (
                      <div>
                        <p className="mb-3 block text-[13px] font-medium text-foreground">
                          Truth Table *
                        </p>
                        <p className="mb-3 text-[11px] text-muted-foreground">
                        ...
                        <div className="overflow-x-auto rounded-lg border border-border">
                          <table className="w-full text-[13px]">
                            <thead>
                              <tr className="bg-secondary">
                                {Array.from({
                                  length: customPieceForm.numInputs,
                                }).map((_, i) => (
                                  <th
                                    key={`in-${i}`}
                                ...
                                {Array.from({
                                  length: customPieceForm.numOutputs,
                                }).map((_, i) => (
                                  <th
                                    key={`out-${i}`}
                                    className="border p-2 text-left"
                            ...
                            <tbody>
                              {Array.from({
                                length: Math.pow(2, customPieceForm.numInputs),
                              }).map((_, rowIdx) => {
                                ...
                                const currentOutputs =
                                  customPieceForm.truthTable[inputKey];
                                  ...
                                  >
                                    ...
                                    {Array.from({
                                      ...
                                      if (
                                        ...
                                        currentValue =
                                          (
                                            currentOutputs as Record<
                                              string,
                                              number
                                            >
                                          )[outKey] ?? 0;
                                      } else if (
                                        customPieceForm.numOutputs === 1 &&
                                        outIdx === 0
                                      ) {
                                        currentValue =
                                          (currentOutputs as number) ?? 0;
                                      }

                                      return (
                                        <td
                                          key={`out-${rowIdx}-${outIdx}`}
                  ...
                  <button
                    ...
                    disabled={
                      !customPieceForm.name.trim() ||
                      !customPieceForm.description.trim()
                    }
                    className="w-full rounded-lg bg-foreground px-4 py-2 text-[13px] font-medium text-background transition-colors hover:bg-foreground/90 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    + Create Custom Piece
                  </button>
                </div>
                        ...
                        {customPieces.map((piece, idx) => {
                          return (
                            <tr key={idx} className="hover:bg-secondary/50">
                              <td className="border p-2 text-[13px] font-medium">
                                {piece.name}
                              </td>
                              <td className="border p-2 font-mono text-[11px]">
                                {piece.cost}
                              ...
                              <td className="border p-2 text-center">
                                <button
                                  onClick={() =>
                                    setViewingCustomPieceTruthTable(idx)
                                  }
                                  className="rounded-lg bg-blue-50/50 px-3 py-1 text-[13px] text-blue-700 transition-colors hover:bg-blue-100"
                                >
                                  View
                                </button>
                              </td>
        ...
        <div className="flex justify-end gap-4 border-t pt-6">
          ...
          <button
            ...
            className="create-puzzle-publish-button rounded-lg bg-foreground px-6 py-2 text-[13px] font-medium text-background transition-colors hover:bg-foreground/90 disabled:opacity-50"
            disabled={isSubmitting}
          >
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <ul className="list-inside list-disc space-y-1 text-[13px]">
                      <li>
                        Define your puzzle&apos;s input and output signals
                      </li>
                      <li>Inputs are the data sources (e.g., A, B, signal1)</li>
                      <li>
                        Outputs are the result signals (e.g., sum, carry,
                        result)
                      </li>
                  ...
                  <div className="rounded-lg bg-secondary/50 p-3">
                    <p className="mb-1 text-[12px] font-semibold">Example:</p>
              ...
              {guideModal === 'solution' && (
                <div className="space-y-3">
                  <div>
                    ...
                    <ul className="list-inside list-disc space-y-1 text-[13px]">
                      <li>
                        <strong>totalCost:</strong> Integer representing total
                        gate cost
                      </li>
                      <li>
                        <strong>eval_map:</strong> Maps input combinations to
                        expected outputs
                      </li>
                      <li>
                        <strong>circuit:</strong> Contains placed components and
                        wires
                      </li>
                      <li>
                        Each input combination must map to correct outputs
                      </li>
                      <li>
                        Wires connect component outputs to component inputs
                      </li>
                    </ul>
                  </div>
                  <div className="rounded-lg bg-secondary/50 p-3">
                    <p className="mb-1 text-[12px] font-semibold">Structure:</p>
  ...
  "circuit": {"placed": [], "wires": []}
              ...
              {guideModal === 'python-tests' && (
                <div className="space-y-3">
                  <div>
                    <p className="mb-2 font-semibold">Python Tests Format</p>
                    <ul className="list-inside list-disc space-y-1 text-[13px]">
                      <li>
                        <strong>REQUIRED:</strong> Define a{' '}
                        <code className="rounded bg-black/20 px-1 text-[11px]">
                          run_tests(solution)
                        </code>{' '}
                        function
                      </li>
                      <li>
                        Create individual test functions to validate the
                        solution
                      </li>
                      <li>
                        Call all test functions from{' '}
                        <code className="rounded bg-black/20 px-1 text-[11px]">
                          run_tests()
                        </code>
                      </li>
                      ...
                      <li>
                        Solution dict contains: placedComponents, wires,
                        totalCost, etc.
                      </li>
                    </ul>
                  </div>
                  <div className="rounded-lg bg-secondary/50 p-3">
                    <p className="mb-1 text-[12px] font-semibold">Example:</p>
                    <p className="font-mono text-[11px] leading-relaxed">{`def run_tests(solution):
  components = solution.get('placedComponents')
  if len(components) < 2:
    raise Exception("Need 2+ gates")`}</p>
              ...
              <div className="overflow-hidden rounded-lg border border-border/60">
                <table className="w-full text-[13px] text-foreground">
                  <thead className="bg-secondary/50 text-[11px] font-medium uppercase text-muted-foreground">
                    <tr>
                      {TRUTH_TABLES[viewingTruthTableFor].inputs.map(
                        (i: string) => (
                      ...
                      {TRUTH_TABLES[viewingTruthTableFor].outputs.map(
                        (o: string) => (
                          <th
                            key={o}
                  ...
                  <tbody className="divide-y divide-border">
                    {TRUTH_TABLES[viewingTruthTableFor].rows.map(
        ...
        <Dialog
          ...
          onOpenChange={(open) =>
            !open && setViewingCustomPieceTruthTable(null)
          }
        >
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                Truth Table:{' '}
                {viewingCustomPieceTruthTable !== null
                  ? customPieces[viewingCustomPieceTruthTable]?.name
                  : ''}
              </DialogTitle>
            </DialogHeader>
            {viewingCustomPieceTruthTable !== null &&
            customPieces[viewingCustomPieceTruthTable] ? (
                ...
                <table className="w-full text-[13px] text-foreground">
                  <thead className="bg-secondary/50 text-[11px] font-medium uppercase text-muted-foreground">
                    <tr>
                      {Array.from({
                        length:
                          customPieces[viewingCustomPieceTruthTable].num_inputs,
                      }).map((_, i) => (
                        <th
                          key={`in-${i}`}
                        ...
                        (_, rowIdx) => {
                          ...
                          return (
                            <tr key={rowIdx} className="divide-x divide-border">
                              ...
                              {Array.from({ length: piece.num_outputs }).map(
                                (_, outIdx) => {
                                  let value = 0;
